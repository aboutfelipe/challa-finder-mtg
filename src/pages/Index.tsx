...
const Index = () => {
  ...
  return <div className="min-h-screen bg-background">
      {/* Hero Section */}
      <div className="relative min-h-screen flex flex-col items-center justify-center bg-cover bg-center bg-no-repeat" style={{
      backgroundImage: `url(${heroBackground})`
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-4 py-12 space-y-12">
          ...
          <div className="text-center space-y-6 mb-12">
            <h1 className="text-5xl md:text-7xl font-bold bg-magic-gradient bg-clip-text text-transparent leading-tight">MTG Challa Finder</h1>
          ...
          {/* Search Results */}
          {(searchResults.length > 0 || !isLoading && lastSearchTerm) && <div className="mt-16">
              {/* Status indicator */}
              {isRealScraping !== null && <div className="mb-4 text-center space-y-2">
                  <div className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-medium ${isRealScraping ? 'bg-green-900/20 text-green-400 border border-green-500/30' : 'bg-red-900/20 text-red-400 border border-red-500/30'}`}>
                    {isRealScraping ? '⚡ Sistema 100% APIs Directas' : '🔴 Usando Datos Simulados'}
                  </div>
                  {searchMethod && <div className="text-xs text-muted-foreground">
          ...
          {/* Store Information */}
          {!isLoading && searchResults.length === 0 && !lastSearchTerm && <div className="mt-16">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
                {getStoreInfo().map(store => <div key={store.name} className="bg-card/50 backdrop-blur-sm border border-border/30 rounded-lg p-4 text-center hover:bg-card/70 transition-all duration-300">
                    <h3 className="font-semibold text-foreground">{store.name}</h3>
                    <a href={store.url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:text-primary-glow transition-colors">
