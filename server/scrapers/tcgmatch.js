export async function scrapeTcgmatch(browser, cardName) {
  ...
  try {
    ...
    // Navigate to TCGMatch
    await page.goto('https://tcgmatch.cl', { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    if (!searchInput) {
      console.log('Could not find search input on TCGMatch');
      return [];
    }
    ...
    if (products.length === 0) {
      console.log('No products found on TCGMatch for:', cardName);
      return [];
    }
    ...
    for (let i = 0; i < Math.min(products.length, 5); i++) {
      try {
        ...
        let productUrl = `https://tcgmatch.cl/cartas/busqueda?q=${encodeURIComponent(cardName)}`;
        if (linkElement) {
          const href = await page.evaluate(el => el.getAttribute('href'), linkElement);
          if (href) {
            productUrl = href.startsWith('http') ? href : `https://tcgmatch.cl${href}`;
          }
        }
        ...
        console.error('Error extracting product info from TCGMatch:', error);
        continue;
      }
    }
    ...
    console.error('Error scraping TCGMatch:', error);
    return [];
  } finally {
    await page.close();
  }
