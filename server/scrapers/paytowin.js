export async function scrapePaytowin(browser, cardName) {
  ...
  try {
    ...
    // Navigate to Pay2Win
    await page.goto('https://www.paytowin.cl', { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    if (!searchInput) {
      console.log('Could not find search input on Pay2Win');
      return [];
    }
    ...
    if (products.length === 0) {
      console.log('No products found on Pay2Win for:', cardName);
      return [];
    }
    ...
    for (let i = 0; i < Math.min(products.length, 5); i++) {
      try {
        ...
        let productUrl = `https://www.paytowin.cl/search?q=${encodeURIComponent(cardName)}`;
        if (linkElement) {
          const href = await page.evaluate(el => el.getAttribute('href'), linkElement);
          if (href) {
            productUrl = href.startsWith('http') ? href : `https://www.paytowin.cl${href}`;
          }
        }
        ...
        console.error('Error extracting product info from Pay2Win:', error);
        continue;
      }
    }
    ...
    console.error('Error scraping Pay2Win:', error);
    return [];
  } finally {
    await page.close();
  }
