...
async function handlePaytowin(request, corsHeaders) {
  ...
  const shopifyUrl = `https://paytowin.cl/search/suggest.json?q=${encodeURIComponent(cardName)}&resources[type]=product&resources[limit]=20`;
...
async function handleTcgmatch(request, corsHeaders) {
  ...
  // FIXED: Changed from tcgmatch.com to tcgmatch.cl and updated API endpoint
  const tcgmatchUrl = `https://tcgmatch.cl/wp-json/wc/v3/products?search=${encodeURIComponent(cardName)}&per_page=20`;
...
async function handleCatlotus(request, corsHeaders) {
  ...
  // FIXED: Using WooCommerce API endpoint instead of custom API
  const catlotusUrl = `https://catlotus.cl/wp-json/wc/v3/products?search=${encodeURIComponent(cardName)}&per_page=20`;
